...
const ChatInput = () => {
  ...
  const handleSendMessage = () => {
    if (messageInput.trim() && user && user.openChatId !== null) {
      ...
      aes.init(
        ...
        getByteLengthUtf16(getOrThrowStr(sharedKey)),
        aesConstants,
      );
      const salt = aes._salt;
      console.log("shared key 4: " + sharedKey);

      const encryptedMessage: string = WordArray.stringifyBase64(
        aes.encryptMessage(messageInput.trim(), aesConstants),
      );
  ...
  return (
    <div className="flex items-center rounded-lg bg-bubbleChat p-2 shadow-md">
      <input
        type="text"
        value={messageInput}
        onChange={(e) => setMessageInput(e.target.value)}
        className="flex-grow rounded-lg border-none bg-bubbleChat p-2 outline-none focus:ring focus:ring-blue-300"
        placeholder="Type your message..."
        onKeyDown={(e) => {
          if (e.key === "Enter") handleSendMessage();
        }}
      />
      <button onClick={handleSendMessage} className="ml-2 px-2 py-1 text-text">
