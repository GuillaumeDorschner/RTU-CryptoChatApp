...
export class KeyIVUtils {
  ...
  computeDerivedKeyRecurse(
    ...
    keySize: number,
    prevBlock: number[] = [],
    resultDerivedKey: WordArray = new WordArray([], 0),
  ): WordArray {
    ...
    const block: number[] = this.narrowToNbArr(
      hasher.appendByteArray(new Uint8Array(password.words)).appendByteArray(new Uint8Array(salt.words)).end(true),
    );
    ...
    return this.computeDerivedKeyRecurse(
      hasher,
      password,
      salt,
      iterations,
      keySize,
      blockIterate,
      resultDerivedKey.concat(new WordArray(blockIterate, 16)),
    );
  }
  ...
  computeDerivedKeyAndIV(
    passwordUtf8: string,
    keySize: number,
    ivSize: number,
    salt: WordArray,
  ): { key: WordArray; iv: WordArray; salt: WordArray } {
