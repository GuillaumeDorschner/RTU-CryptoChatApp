...
class AESConstantsImpl {
  ...
  computeCounter(x: number, xi: number, mults: multiplications, doubles: number[]): { x: number; xi: number } {
    return !x
      ? { x: 1, xi: 1 }
      : { x: mults.x2 ^ doubles[doubles[doubles[mults.x8 ^ mults.x2]]], xi: xi ^ doubles[doubles[xi]] };
  }
  ...
  accumulateElements(
    existingConstants: AESConstants,
    sboxElement: sboxElement,
    subMixElements: subMixElements,
    invSubMixElements: invSubMixElements,
  ): AESConstants {
    return {
  ...
  computeAESConstantsRecurse(
    ...
    x: number = 0,
    xi: number = 0,
    resultingAESConstants: AESConstants = this.startConstants,
  ): AESConstants {
    ...
    const aesConstants: AESConstants = this.accumulateElements(
      resultingAESConstants,
      sboxElement,
      subMixElements,
      invSubMixElements,
    );
